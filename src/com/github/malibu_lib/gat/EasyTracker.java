...
import com.github.malibu_lib.gat.internal.GoogleAnalyticsTrackerDelegate;
import com.github.malibu_lib.gat.internal.GoogleAnalyticsTrackerDelegateImpl;
import com.github.malibu_lib.gat.internal.ParameterLoader;
import com.github.malibu_lib.gat.internal.ParameterLoaderImpl;
import com.google.android.apps.analytics.Item;
import com.google.android.apps.analytics.Transaction;

/**
 * EasyTracker is a class designed to easy the burden of adding tracking code to
 * your application. Simply add this class to your application and extend
 ...
 * extend that instead of FragmentActivity. The code for TrackedFragmentActivity
 * should be the same as the code in TrackedActivity.
 * <p>
 * You can turn on tracking by providing a String resource of the name
 * ga_api_key with a value of your account id (form UA-12345-6). You can provide
 * various parameters as String, Bool or Integer resources (such as sampleRate)
 * as well. Just use the proper type for the parameter (String for String, Bool
 * for boolean and Integer for int).
 * <p>
...
public class EasyTracker {
    ...
    // If true, hits will be generated normally, but will not actually be sent
    // to
    ...
    // application installs will report while a value of 0 means none will
    // report.
    ...
    // is started (via the trackActivityStart method) and an empty event when
    // the
    ...
    // The number of Activities that have had their onStart method called but
    // not
    ...
    private void loadParameters() {
        ...
        if (gaAccountId != null) {
            ...
            appVersion = parameterFetcher.getVersion();
    ...
    /**
     * Sets the context to use to the applicationContext of the Context ctx. If
     * the input is not null, this method will then go on to initialize the
     * EasyTracker Class with parameters from the resource files. If there is an
     * accountId specified, this method will enable Google Analytics tracking
     * and start up the database Thread. If not, it will leave tracking
     * disabled.
     * 
     ...
     * event to Google Analytics if autoActivityTracking is false to ensure
     * proper application-level tracking. Developers should not call this method
     ...
     */
    public void trackActivityStart(final String activityName) {
        activitiesActive++;
        ...
        queueToTrackerThreadIfEnabled(new Runnable() {
            ...
            public void run() {
                if (startASession) {
                    getGoogleAnalyticsTracker().startNewSession(gaAccountId, gaDispatchPeriod,
                            gaContext);
                    getGoogleAnalyticsTracker().setCustomVar(1, "android",
                            android.os.Build.VERSION.RELEASE, 1);
                    getGoogleAnalyticsTracker()
                            .setCustomVar(2, "device", android.os.Build.MODEL, 1);
                    getGoogleAnalyticsTracker().setCustomVar(3, "version", appVersion, 1);
                    if (!autoActivityTracking) {
                        // We send an empty event so we get accurate
                        // time-on-site info.
                ...
                if (autoActivityTracking) {
                    if (activityName.endsWith("Activity")) {
                        getGoogleAnalyticsTracker()
                                .trackPageView(
                                        getActivityName(activityName.substring(0,
                                                activityName.length() - 8)));
                    } else {
                        getGoogleAnalyticsTracker().trackPageView(getActivityName(activityName));
                    }
                }
            }
        });
     ...
     * an empty event to Google Analytics. Note that this method should be
     * called from the Activity's onStop callback.
     * 
     * @param activity the Activity that is to be tracked
     */
    public void trackActivityStop() {
        ...
        queueToTrackerThreadIfEnabled(new Runnable() {
            @Override
            public void run() {
                // We send an empty event so we get accurate time-on-page/site
                // info.
                getGoogleAnalyticsTracker().trackEvent("", "", "", 0);
            }
        });
    ...
    /**
     * Look up the Activity's display name (as defined in a String resource
     * named for the Activity's canonicalName).
     * 
    ...
    private String getActivityName(String activityName) {
        String canonicalName = activityName;
    ...
    // GoogleAnalyticsTracker with the exception that they all are run on a
    // single
     ...
     * the same orderId already exists, it will be replaced with this
     * Transaction.
     * 
    ...
    /**
     * Dispatch up to 30 queued hits to the Google Analytics servers, but only
     * if another dispatch is not in progress.
     */
    ...
    /**
     * Set the campaign referral to the values in the input. If the input is not
     * valid, the method fails. If the input is valid, the referrer information
     * is changed and a new session is started. Each hit after a successful call
     * to setReferrer will have the referrer information attached.
     * 
    ...
    public void startNewSession() {
        queueToTrackerThreadIfEnabled(new Runnable() {
            @Override
            public void run() {
                // If this gets run, we know that gaAccountId and gaContext are
                // not null
                // as the method queueToDbThreadIfEnabled will check the
                // gaEnabled flag
                // before queuing anything. That flag, in turn is set to true
                // only if
    ...
    /**
     * Sends all the pending Transactions and Items to dispatch. Once this
     * method is called, all the Transactions and Items added previously will
     * not be cleared by a clearTransactions call and will eventually be
     * dispatched to the GoogleAnalytics servers.
     */
